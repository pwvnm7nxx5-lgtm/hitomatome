...
type WidgetMode = 'today' | 'calendar'
...
const widgetModeFromUrl = (): WidgetMode | null => {
  const mode = new URLSearchParams(window.location.search).get('widget')
  return mode === 'today' || mode === 'calendar' ? mode : null
}
...
function App() {
  const [widgetMode] = useState<WidgetMode | null>(widgetModeFromUrl)
  const isWidget = widgetMode !== null
  const [view, setView] = useState<View>(widgetMode === 'calendar' ? 'calendar' : 'today')
...
[
  ...
  const widgetUrl = (mode: WidgetMode) => `${window.location.origin}${window.location.pathname}?widget=${mode}`
  ...
  return <div className={`app-shell ${isWidget ? `widget-shell widget-${widgetMode}` : ''}`}>
    ...
    <main className={isWidget ? 'widget-main' : undefined}>{!isWidget && <nav aria-label="メインメニュー"><button aria-current={view === 'today' ? 'page' : undefined} className={view === 'today' ? 'active' : ''} onClick={() => setView('today')}>今日 <b>{todayItems.length}</b></button><button aria-current={view === 'calendar' || view === 'day' ? 'page' : undefined} className={view === 'calendar' || view === 'day' ? 'active' : ''} onClick={() => setView('calendar')}>カレンダー</button><button aria-current={view === 'all' ? 'page' : undefined} className={view === 'all' ? 'active' : ''} onClick={() => setView('all')}>すべて <b>{openItems.length}</b></button><button aria-current={view === 'completed' ? 'page' : undefined} className={view === 'completed' ? 'active' : ''} onClick={() => setView('completed')}>完了済み</button><button aria-current={['more', 'connections', 'inbox', 'import', 'help'].includes(view) ? 'page' : undefined} className={['more', 'connections', 'inbox', 'import', 'help'].includes(view) ? 'active' : ''} onClick={() => setView('more')}>その他</button></nav>}
      <section className="content">
        ...
        {view === 'help' && <div className="help"><button className="back-button" onClick={() => setView('more')}>← その他へ戻る</button><h2>使い方</h2><section><h3>普段の追加</h3><p>画面上部へ「明日 16時 職員会議」のように入力します。候補を確認し、予定またはタスクとして保存してください。</p></section><section><h3>未整理とは？</h3><p>日付が決まっていないものや、ChatGPTから取り込んでまだ確認していない項目を一時的に置く場所です。内容を編集して日付を入れると、カレンダーや「すべて」で確認できます。</p></section><section><h3>カレンダーと完了済み</h3><p>カレンダーの日付には予定名が表示されます。日付を押すと日別画面へ移動し、その日の予定確認と追加ができます。完了済み画面では、完了した項目を未完了へ戻せます。</p></section><section><h3>PCのウィジェット表示</h3><p>「その他」から今日の予定ウィジェット、またはカレンダーウィジェットを開けます。PCでは開いたページをChromeやEdgeの「ショートカットを作成」から「ウィンドウとして開く」にすると、デスクトップに小さい予定欄として置けます。</p></section><section><h3>スマホのホーム画面へ追加</h3><p>iPhoneはSafariの共有ボタンから「ホーム画面に追加」、AndroidはChromeメニューから「ホーム画面に追加」または「アプリをインストール」を選びます。ウィジェットURLを開いてから追加すると、今日の予定版やカレンダー版を直接開けます。</p></section><section><h3>プリントから取り込む</h3><ol><li>プリントに児童名・住所・連絡先などがあれば、撮影前または画像編集で必ず隠します。</li><li>ChatGPTへ画像を送り、下の指示文を貼り付けます。</li><li>返ってきたJSONを「ChatGPTから取込」に貼り付けます。</li><li>未整理で日付や内容を確認してから使います。</li></ol><div className="prompt-box"><pre>{prompt}</pre><button onClick={copyPrompt}>{copied ? 'コピーしました' : '指示文をコピー'}</button><span className="sr-only" aria-live="polite">{copied ? '指示文をコピーしました' : ''}</span></div></section><section className="caution"><h3>大切な注意</h3><p>ChatGPTへ送る前に、児童・保護者・職員の個人情報を必ず除いてください。共有PCでは、同じブラウザを使う人に予定が見える可能性があります。</p></section><section><h3>保存とバックアップ</h3><p>データは、このブラウザ内だけに保存されます。端末の紛失やブラウザデータ削除に備えて、定期的にバックアップしてください。</p><div className="backup-actions"><button onClick={exportBackup}>バックアップを保存</button><label className="file-button">バックアップを復元<input type="file" accept="application/json,.json" onChange={(event) => restoreBackup(event.target.files?.[0])} /></label></div></section></div>}
      </section></main>
